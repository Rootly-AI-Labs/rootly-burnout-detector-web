...
from .base import Base, engine  # import engine so we can detect the backend

IS_SQLITE = make_url(str(engine.url)).get_backend_name() == "sqlite"
...
class Organization(Base):
    ...
    # ---- Constraints (dialect-aware) ----
    if IS_SQLITE:
        # SQLite uses length() and has no regex operator; do a coarse domain check
        name_len_check = CheckConstraint("length(name) >= 2", name="organization_name_min_length")
        domain_check   = CheckConstraint("instr(domain, '.') > 0", name="organization_domain_format_sqlite")
    else:
        # Postgres: keep your original semantics
        name_len_check = CheckConstraint("char_length(name) >= 2", name="organization_name_min_length")
        domain_check   = CheckConstraint(r"domain ~ '^[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$'", name="organization_domain_format")

    __table_args__ = (name_len_check, domain_check)
